...
import { ClassT } from '../types/utilityTypes'

const rootStore = Symbol('rootStore')

type Separator<T> = [Class: T, name: string]

// 동적 구분자(Separator)에 따른 데이터 저장
class DynamicStore {
  @observable private [rootStore] = new WeakMap<ClassT, Map<string, InstanceType<ClassT>>>()

  constructor() {
    intercept(this[rootStore], change => {
      return change
    })
  }

  @action reg<T extends ClassT>(
  ...
  @action del<T extends ClassT>([Service, name]: Separator<T>) {
    return !!this[rootStore].get(Service)?.delete(name)
  }

  @action deleteRoot<T extends ClassT>(Service: T) {
